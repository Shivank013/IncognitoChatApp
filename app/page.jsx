            ...
            <div className="flex flex-col w-[100%] h-[85%]">
                  ...
                  >
                    {messages?.map((msg, index) => (
                      ...
                      {
                        !(msg.from === socket.id) ?
                        <div><div className=" text-red-400 text-[8px] sm:text-[15px]">id:{msg.from} </div>{msg.message} <div className=" text-end text-[6px] sm:text-[12px] font-normal">{msg.time}</div></div> : 
                        <span>{msg.message} <div className=" text-end text-[6px] sm:text-[12px] font-normal">{msg.time}</div></span>
                      }
                      </div>
                    ))}
                  </div>
